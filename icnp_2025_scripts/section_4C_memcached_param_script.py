...
elif POLICY.lower() == "caladan_no_oc":
    SCHEDULER = "ias"
...
# Updating the core allocator and overload controller files
if True:
...
# Update the memcached code
print("replacing memcached.c and mcclient.c")
for node in NODES:
    cmd = "scp -P 22 -i {} -o StrictHostKeyChecking=no ../replace/memcached.c"\
            " {}@{}:~/{}/memcached/"\
            .format(KEY_LOCATION, USERNAME, node, ARTIFACT_PATH)
    execute_local(cmd)
    cmd = "scp -P 22 -i {} -o StrictHostKeyChecking=no ../replace/mcclient.cc"\
            " {}@{}:~/{}/memcached-client/"\
            .format(KEY_LOCATION, USERNAME, node, ARTIFACT_PATH)
    execute_local(cmd)
...
# Building Shenango/Caladan apps
print("Building Shenango/Caladan applications...")
...
cmd = "cd ~/{}/{} && sudo ./iokerneld {} interval {:d} 0-8,10-18 2>&1 | ts %s > iokernel.node-0.log".format(ARTIFACT_PATH, KERNEL_NAME, SCHEDULER, CALADAN_INTERVAL)
...
output_prefix += "_nconn_{:d}".format(NUM_CONNS)
