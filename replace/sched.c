...
atomic64_t total_reductions;
atomic_t credit_reduction;
atomic64_t bad_actions;
	...
	if (cfg_breakwater_prevent_parks) {
		...
		if (notified_breakwater && old_C_issued - current_C_issued >= breakwater_park_target) {
			...
			if (!notified_breakwater) {
				...
				if (breakwater_park_target > old_C_issued) { // could check for race condition later, seems more iffy
					atomic64_inc(&bad_actions); // this does happen... what's the best approach? is it even reasonable to target 0 issued?
				}
				atomic64_inc(&total_reductions);
				atomic_fetch_and_add(&credit_reduction, breakwater_park_target);
