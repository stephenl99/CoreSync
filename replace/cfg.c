...
static int parse_coresync_r(const char *name, const char *val)
{
	long tmp;
	int ret;
...
static const struct cfg_handler cfg_handlers[] = {
	...
	{ "coresync_r", parse_coresync_r, false },
};
...
int cfg_load(const char *path)
{
	...
	if (cfg_coresync_enable && cfg_coresync_R == 0) {
		log_err("CoreSync: Invalid value for parameter R, should be positive.");
		ret = -EINVAL;
		goto out;
	}
